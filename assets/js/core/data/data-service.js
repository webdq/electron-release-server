angular.module("app.core.data.service", ["ngSails"]).service("DataService", [
  "$sails",
  "$q",
  "$log",
  "$http",
  ...
  function (
    ...
    var showAttributeWarnings = function (response) {
      ...
      _.forEach(response.data.invalidAttributes, function (
        attribute,
        attributeName
      ) {
        var warningMessage = "";
    ...
    self.createVersion = function (version) {
      ...
      if (!version.availability) {
        Notification.error({
          title: errorTitle,
          message:
            "Availability date must be greater than or equal to the current date",
        });

        return $q.reject();
      }

      return $http.post("/api/version", version).then(
        function (response) {
     ...
     *                               Contains the response object.
     */
    self.createFlavor = function (flavor) {
      if (!flavor) {
        throw new Error("A flavor object is required for creation");
      }

      return $http.post("/api/flavor", flavor).then(
        function (response) {
    ...
    self.updateVersion = function (version) {
      ...
      if (!version.availability) {
        Notification.error({
          title: errorTitle,
          message:
            "Availability date must be greater than or equal to the version creation date",
        });
      ...
      return $http
        .post("/api/version/" + version.id, _.omit(version, ["assets"]))
        .then(
          function (response) {
    ...
    self.deleteVersion = function (versionId) {
      ...
      return $http.delete("/api/version/" + versionId).then(
    ...
    $sails.on("version", function (msg) {
      ...
      var version = normalizeVersion(msg.data || msg.previous);

      var index;
      var notificationMessage = version
        ? version.name + " (" + version.flavor.name + ")"
        : "";
    ...
    $sails.on("flavor", function (msg) {
      ...
      if (msg.verb === "created") {
        ...
        self.availableFlavors = _.concat(
          [],
          [msg.data.name],
          self.availableFlavors
        ).sort();
        ...
        self.availableFlavors = self.availableFlavors.filter(function (flavor) {
          return flavor !== msg.previous.name;
        });
    ...
    self.updateAsset = function (asset) {
      ...
      if (!asset.id) {
        throw new Error(
          "The passed asset object must have been submitted to the database in order to be updated"
        );
      }

      return $http.post("/api/asset/" + asset.id, asset).then(
        function (response) {
    ...
    self.deleteAsset = function (name) {
      ...
      return $http.delete("/api/asset/" + name).then(
        function success(response) {
    ...
    $sails.on("asset", function (msg) {
      ...
      var asset = normalizeAsset(msg.data);
    ...
    self.initialize = function () {
      ...
      return $q
        ...
        .then(function (responses) {
          var versions = responses[0];
          var channels = responses[1];
          var flavors = responses[2];
          self.data = versions.data.items;
          self.availableChannels = channels.data.map(function (channel) {
            return channel.name;
          });
          self.availableFlavors = flavors.data
            .map(function (flavor) {
              return flavor.name;
            })
            .sort();
     ...
     */
    self.checkAvailability = function (version) {
      return new Date(version.availability) <= new Date();
    };
     ...
     */
    self.getLatestReleases = function (platform, archs, channel, flavor) {
      ...
      if (
        !self.availableFlavors ||
        self.availableFlavors.indexOf(flavor) === -1
      ) {
        return;
      }

      var versions = _.chain(self.data)
        ...
        .filter(function (version) {
          return version.flavor.name === flavor;
        })
        .value();
