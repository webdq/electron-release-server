angular
  .module("app.admin.edit-version-modal", [])
    ...
    function (
      ...
      $scope.openAddAssetModal = function () {
        var modalInstance = $uibModal.open({
          animation: true,
          templateUrl:
            "js/admin/add-version-asset-modal/add-version-asset-modal.html",
      ...
      $scope.openEditAssetModal = function (asset) {
        var modalInstance = $uibModal.open({
          animation: true,
          templateUrl:
            "js/admin/edit-version-asset-modal/edit-version-asset-modal.html",
      ...
      $scope.acceptEdit = function () {
        DataService.updateVersion($scope.version).then(
      ...
      $scope.makeAvailable = function () {
        var updatedVersion = _.assign({}, $scope.version, {
          availability: moment().startOf("second").toDate(),
        });

        DataService.updateVersion(updatedVersion).then(
          $uibModalInstance.close,
          function () {}
        );
      };
      ...
      $scope.deleteVersion = function () {
        DataService.deleteVersion(version.id).then(
          function success(response) {
            $uibModalInstance.close();
          },
